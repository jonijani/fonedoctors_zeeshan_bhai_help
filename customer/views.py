...
def update_client_details(request,id):
    ...
    if request.method == "POST":
        ...
        created_date = datetime.datetime.now()
        #created_by = request.user#created_by new variable to bring authrosed user in request.user
        customer_update_data = Customer.objects.filter(id=id).update(first_name=first_name,last_name=last_name,phone=phone, landline=landline,email=email,address=address, postcode=postcode , created_date=created_date  )
